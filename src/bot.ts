...
import { BackLog, BotInterface, KoukokuServer, Web } from '.'
...
export class Bot implements AsyncDisposable, BotInterface {
  ...
  private async acceptKoukoku(data: Buffer): Promise<void> {
    if (this.threshold < data.byteLength) {
      ...
      const matched = text.match(Bot.BackLogRE)
      const bound = [this.appendLogAsync.bind(this, text), this.locateBacklogsAsync.bind(this, matched)]
      await bound[+(!!matched)]()
    }
  }
