...
import { BackLog, BotInterface, DeepL, GitHub, IgnorePattern, KoukokuServer, Speech, Unicode, Web, compileIgnorePattern, isDeepLError, isGitHubResponse, selectBodyOfBackLog, shouldBeIgnored } from '.'
...
export class Bot implements AsyncDisposable, BotInterface {
  ...
  private async translateAsync(match: RegExpMatchArray): Promise<void> {
    ...
    if (isDeepLError(r))
      await this.sendAsync(r.message)
    else
      for (const t of r.translations) {
        ...
        await this.sendAsync(`[${name}から${to}翻訳] ${text == mutated ? text : Unicode.escape(text)}`)
      }
  }
