...
import { BackLog, BotInterface, Speech, replaceVariables } from '.'
...
export class Web implements Disposable {
  ...
  private readonly pending = new WeakMap<WebSocketClient, BackLog[]>()
  ...
  private enqueuePending(client: WebSocketClient, ...data: BackLog[]): void {
  ...
  private notifyWebClient(client: WebSocketClient): void {
    this.bot.notifyWebClient((data: BackLog[]) => {
